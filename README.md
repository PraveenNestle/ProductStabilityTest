...
Open `frontend/dist-mockup/index.html` in a browser (or `Stability_Capture_Mockup.html` from the delivery folder). It runs entirely in the browser with 120 seeded observations across 5 ARs. Switch persona (scientist, reviewer, admin) from the top right to see role-based access change what the app offers.
...
`backend/infra/deploy.sh dev` registers the two Entra apps (API with app roles, SPA), deploys `main.bicep` (one storage account, one Web App), builds the React app with the tenant settings, zips `frontend/dist` and `backend/api` into one Node Web App and registers the redirect URI. The script uses an explicit deployment name and `npm install`, so it works with this repository before lockfiles have been generated.

The Bicep parameter template is [backend/infra/main.parameters.example.json](backend/infra/main.parameters.example.json). Copy it to a local file, replace the Entra tenant and API audience values, then deploy with `az deployment group create --resource-group <resource-group> --name stability-capture-dev --template-file backend/infra/main.bicep --parameters @backend/infra/main.parameters.json`. Keep the copied file out of source control when it contains environment-specific values.

On Windows, run the Bash script from Git Bash or WSL. Azure CLI must be logged in, Node.js 22 must be installed, and the account must have permission to create a resource group, App Service, Storage account, Application Insights, and role assignments. The Web App is configured for the `NODE|22-lts` App Service runtime. After deployment, verify `<web-app-url>/api/health`, then import reference data and assign the Entra app roles.
