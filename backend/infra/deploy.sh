...
DEPLOYMENT_NAME="stability-capture-$ENV"
...
az deployment group create -n "$DEPLOYMENT_NAME" -g "$RG" -f "$ROOT/backend/infra/main.bicep" \
  -p env="$ENV" entraTenantId="$TENANT_ID" entraApiAudience="api://stability-capture-api-$ENV" -o none
WEB=$(az deployment group show -g "$RG" -n "$DEPLOYMENT_NAME" --query properties.outputs.webAppUrl.value -o tsv)
